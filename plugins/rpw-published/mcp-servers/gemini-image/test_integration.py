...
Requires Vertex AI config in environment (load via dev.env): GOOGLE_CLOUD_PROJECT
plus a valid gcloud ADC session.
...
def _has_project():
    """True if Vertex AI is configured (GOOGLE_CLOUD_PROJECT set)."""
...
@unittest.skipUnless(_has_project(), "GOOGLE_CLOUD_PROJECT not set — skipping integration tests")
...
class TestClientSetup(unittest.TestCase):
    def test_client_builds_with_project(self):
        """Verify a Vertex AI client builds from the configured project."""
        import mcp_server
        client = mcp_server._get_client()
        self.assertIsNotNone(client)


class TestClientRequiresProject(unittest.TestCase):
    """No network or credentials required — runs unconditionally."""

    def test_client_fails_without_project(self):
        import mcp_server
