...
1. **Validate state**: Check `~/.claude/rpw-published.local.md` exists and has required fields (github_owner, github_repo, github_repo_url, github_project_number, etc.). Verify repo is reachable with current GitHub auth.
...
3. **If convert selected**: Run epic-to-milestone bootstrap (use human-todo skill):
...
4. **Save**: Write final config to `~/.claude/rpw-published.local.md`.

Use the human-todo skill for detailed steps and safety rules.
