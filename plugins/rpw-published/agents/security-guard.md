...
`SECURITY_GUARD_SYSTEM_PROMPT` in `libs/rpw_runtime/src/rpw_runtime/agents/subagents.py`, is
canonical for the **DeepAgents/LangGraph runtime path** that `rpw build --live` executes. The two
are DELIBERATELY DIFFERENT (this path carries Claude frontmatter + a prose table result format; the
runtime path is terse and harness-neutral). A role-behavior change must be applied to BOTH — a repo
validation (`tests/test_repo_validations.py::TestBuildSubagentTwins`) enforces the cross-reference.
See `docs/architecture/build-subagents-langgraph.md`. -->
