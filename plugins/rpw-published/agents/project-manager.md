...
5. Issue management: create, update, and close issues on behalf of the user via `gh` CLI

**Tool Mapping:**
- Use `gh` CLI for issue management (GitHub Issues). If unavailable, fall back to git log and file analysis.
- Use generic project terminology: epics, features, tasks (not tool-specific jargon)

**Analysis Process:**

For project health:
1. Run `gh issue list --state open` for open work
2. Run `gh issue list --label "status: in-progress"` for active work
3. Run `gh issue list --label "status: blocked"` for blocked issues
4. Run `gh issue list --state open --limit 10` for available work
