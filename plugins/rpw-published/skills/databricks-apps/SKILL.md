...
Set `$STARTER_TEMPLATE_REPO` to the git URL of the repo that holds your Apps
starter template (the reference implementation below assumes a
`databricks/apps_starter` subtree — adjust the sparse path if yours differs).

```bash
# Clone the starter-template repo (shallow, sparse) and copy the template
git clone --depth 1 --filter=blob:none --sparse "$STARTER_TEMPLATE_REPO" /tmp/apps-starter-clone
cd /tmp/apps-starter-clone && git sparse-checkout set databricks/apps_starter
cp -r databricks/apps_starter/* databricks/apps_starter/.* <TARGET_PROJECT_DIR>/
rm -rf /tmp/apps-starter-clone
```

Source: `$STARTER_TEMPLATE_REPO` → `databricks/apps_starter`
...
Each shell request wraps the command with a CWD marker (`__RCP_CWD_<uuid>__=`) appended after execution. The marker is stripped from output and used to persist the working directory across requests. Session state is stored in SQLite (`/tmp/apps_starter_session_state.sqlite3`).
...
git clone "$STARTER_TEMPLATE_REPO" apps-starter-repo
cd apps-starter-repo/databricks/apps_starter
...
env:
  ...
  - name: SESSION_STATE_DB_PATH
    value: "/tmp/apps_starter_session_state.sqlite3"
```
